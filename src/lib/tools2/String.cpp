...
static int ucstrcmp(const String &as, const String &bs)
{
...
inline int String::ref() const
{
	String *self = const_cast<String *>(this);
	return (*self->_d)++;
}

inline int String::unref() const
{
	String *self = const_cast<String *>(this);
	return --(*self->_d);
}

inline WChar *String::data()
{ return _d + 3; }

inline const WChar *String::data() const
{ return _d + 3; }

inline unsigned short String::len() const
{ return _d[1]; }

inline unsigned short &String::len_r()
{ return _d[1]; }
...
bool String::assureSize(uint asz)
{
...
void String::blkmov(int from, int offset)
{
	if (offset > 0) {
...
String::String(const unsigned short *unicodeAsUShorts, uint len)
{
	init();
...
void String::truncate(uint newLen)
{
	if (newLen >= length())
		return;
...
String String::format(const char *cformat, ...)
{
	char *sbuf = NULL;
	int nchars = 0;
	int bufsz = 100;
...
int String::indexOf(char c, int from, bool caseSensitive) const
{
...
int String::indexOf(const String &str, int from, bool caseSensitive) const
{
	const int lthis = static_cast<int>(length());
	if (from < 0)
		from += lthis;
...
int String::lastIndexOf(char c, int from, bool caseSensitive) const
{
	const int lthis = static_cast<int>(length());
	if (from < 0)
		from += lthis;
...
int String::lastIndexOf(const String &str, int from, bool caseSensitive) const
{
	const int lstr = static_cast<int>(str.length());
	if (lstr == 0)
		return -1;
...
int String::findCharOf(const char *chset) const
{
	assert(chset != NULL);
...
String String::left(uint len) const
{
...
String String::right(uint len) const
{
	const uint lthis = length();
	if (lthis == 0 || len == 0)
		return String::null;
...
String String::mid(uint index, uint len) const
{
	const uint lthis = length();
...
String *String::split(const String &sep, int *n) const
{
	*n = 0;
	int bufsz = 8;
	int count = 0;
	String *sl = NEW_OBJARR String[bufsz];
	if (sl == NULL)
		return NULL;
	...
	while ((pos2 = indexOf(sep, pos1)) != -1) {
		...
		if (count == bufsz) {
			bufsz *= 2;
			String *bigbuf = NEW_OBJARR String[bufsz];
			if (bigbuf == NULL) {
				DELETE_OBJARR(sl);
				return NULL;
			}
...
String &String::appendUcs2Code(WChar wc)
{
	clone();
...
String &String::prependUcs2Code(WChar wc)
{
	clone();
...
String &String::remove(uint index, uint len)
{
	uint olen = length();
	if (index >= olen) {
		// range problem
...
String &String::setNumber(long n, int base)
{
	char sbuf[65];
	char *p = &sbuf[64];
	int  len = 0;
...
String &String::setNumber(unsigned long n, int base)
{
	char sbuf[65];
	char *p = &sbuf[64];
	int len = 0;
...
String &String::setNumber(double n, char f, int prec)
{
	char format[20];
...
String String::number(unsigned long n, int base)
{
	String s;
	s.setNumber(n, base);
	return s;
...
const char *String::toLatin1() const
{
	String *self = const_cast<String *>(this);
...
String &String::setLatin1(const char *str, int len)
{
	clear();
...
String &String::setUnicode(const unsigned short *unicodeAsUShorts, uint len)
{
	clear();
...
String String::fromLatin1(const char *str, int len)
{
	String res;
...
String String::fromUtf8(const char *utf8, int len)
{
	String res;
...
String String::fromUcs2(const char *ucs2buf, int len, bool bigEndian)
{
	String res;
...
bool MyTools::operator==(const String &s1, const String &s2)
{
...
bool MyTools::operator==(const String &s1, const char *s2)
{
	if (s2 == NULL)
		return s1.isNull();
...
bool MyTools::operator==(const char *s1, const String &s2)
{ return s1 == s2; }
