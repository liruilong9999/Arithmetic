...
inline ConfigEntry *ConfigGroup::findEntry(const String &key) const
{
...
ConfigEntry *ConfigGroup::addEntry(const String &key, const String &value)
{
...
ConfigGroup::ConfigGroup(const String &name)
{
	_sibling = NULL;
...
ConfigGroup ConfigGroup::fromString(const String &str, char sepChar)
{
	ConfigGroup res;
	int posFieldBegin = 0;
...
void ConfigGroup::setValue(const String &key, const String &value)
{
...
void ConfigGroup::setValue(const String &key, long value)
{
	ConfigEntry *pent = findEntry(key);
	if (pent != NULL)
		pent->val.setNumber(value);
	else
		addEntry(key, String::number(value));
}

void ConfigGroup::setValue(const String &key, unsigned long value)
{
	ConfigEntry *pent = findEntry(key);
	if (pent != NULL)
		pent->val.setNumber(value);
...
void ConfigGroup::setValue(const String &key, double value)
{
	ConfigEntry *pent = findEntry(key);
	if (pent != NULL)
		pent->val.setNumber(value);
...
String ConfigGroup::getValue(const String &key, const String &defaultValue) const
{
	ConfigEntry *pent = findEntry(key);
	if (pent != NULL)
		return pent->val;
...
long ConfigGroup::getIntValue(const String &key, const long defaultValue) const
{
	ConfigEntry *pent = findEntry(key);
	if (pent == NULL) {
		LOG_E(MYTAG, "no such key: " + key);
...
unsigned long ConfigGroup::getUIntValue(const String &key, unsigned long defaultValue) const
{
	ConfigEntry *pent = findEntry(key);
	if (pent == NULL) {
		LOG_E(MYTAG, "no such key: " + key);
...
double ConfigGroup::getFloatValue(const String &key, double defaultValue) const
{
	ConfigEntry *pent = findEntry(key);
	if (pent == NULL) {
		LOG_E(MYTAG, "no such key: " + key);
...
String ConfigGroup::toString(char sepChar) const
{
	String s;
	s.append(_groupName);
	ConfigEntry *pent = _child;
...
{
	if (_curGroup != NULL)
		_curGroup->setValue(String::fromUtf8(key), String::fromUtf8(val));
}
...
bool Config::open(const String &fileName)
{
	_fileName = fileName;
...
void Config::setGroup(const String &name)
{
	ConfigGroup *pgrp = _grps;
...
void Config::removeGroup(const String &name)
{
	ConfigGroup *pgrp = _grps;
	ConfigGroup **after = &_grps;
	while (pgrp != NULL) {
...
void Config::setValue(const String &key, const String &val)
{
	if (_curGroup != NULL) {
...
void Config::setValue(const String &key, int val)
{
	setValue(key, static_cast<long>(val));
}

void Config::setValue(const String &key, unsigned int val)
{
	setValue(key, static_cast<unsigned long>(val));
}

void Config::setValue(const String &key, long val)
{
	if (_curGroup != NULL) {
		_curGroup->setValue(key, val);
		_dirty = true;
...
void Config::setValue(const String &key, unsigned long val)
{
	if (_curGroup != NULL) {
		_curGroup->setValue(key, val);
		_dirty = true;
...
void Config::setValue(const String &key, double val)
{
	if (_curGroup != NULL) {
		_curGroup->setValue(key, val);
		_dirty = true;
...
long Config::getIntValue(const String &key, long defaultValue) const
{
	if (_curGroup != NULL)
...
unsigned long Config::getUIntValue(const String &key, unsigned long defaultValue) const
{
	if (_curGroup != NULL)
...
double Config::getFloatValue(const String &key, double defaultValue) const
{
	if (_curGroup != NULL)
...
String Config::toString(char sepChar) const
{
	String s;
	if (_curGroup != NULL)
